...
de desenvolvimento. Antes de rodar, crie o banco uma única vez:
...
As variáveis abaixo podem ser sobrescritas por variáveis de ambiente
(TEST_DATABASE_URL) se o usuário/senha do seu Postgres forem diferentes.
"""
import os

# IMPORTANTE: definir as variáveis ANTES de importar o app, porque o
# config.py lê o ambiente no momento do import.
...
# de vezes. Desligamos só no ambiente de teste.
...
def criar_admin(username="admin_t", password="senha12345"):
    """Admin não tem rota pública de cadastro; criamos direto no banco."""
    db = SessionLocal()
    try:
        user = User(
